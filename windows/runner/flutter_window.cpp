...
bool FlutterWindow::OnCreate() {
  ...
  // Register JujoStream native streaming + gamepad plugins
  auto *raw_stream = flutter_controller_->engine()->GetRegistrarForPlugin(
      "StreamingPluginWin");
  if (raw_stream) {
    streaming_registrar_ = std::make_unique<flutter::PluginRegistrarWindows>(raw_stream);
    jujostream::StreamingPluginRegisterWithRegistrar(streaming_registrar_.get());
  }
  auto *raw_gp = flutter_controller_->engine()->GetRegistrarForPlugin(
      "GamepadPluginWin");
  if (raw_gp) {
