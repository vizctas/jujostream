...
.PHONY: help build-apk build-aab release-apk release-apk-dry-run clean
...
$(SHA_OUT): $(APK_OUT)
	powershell -NoProfile -Command "$$lines = @(); $$lines += ((Get-FileHash -Algorithm SHA256 -LiteralPath '$(APK_OUT)').Hash.ToLowerInvariant() + '  $(APK_NAME)'); $$lines += ((Get-FileHash -Algorithm SHA256 -LiteralPath '$(AAB_OUT)').Hash.ToLowerInvariant() + '  $(AAB_NAME)'); Set-Content -LiteralPath '$(SHA_OUT)' -Value $$lines -Encoding ascii"
